...
Shader "Custom/SpatialMesh"
{
	...
	Pass
	{
		...
		v2g vert(appdata_base v)
		{
			...
			o.distFromCam.xyz = (float3)0;

			fixed4 worldPos = mul(unity_ObjectToWorld, v.vertex);
			fixed dist = length(worldPos);//length(_WorldSpaceCameraPos - worldPos);
			o.distFromCam.w = dist;
		...
		void geom(triangle v2g i[3], inout TriangleStream<g2f> triStream)
		{
			...
			for (uint idx = 0; idx < 3; ++idx)
			{
				...
				o.dist.xyz = distScale[idx] * o.viewPos.w * wireScale;
				o.dist.w = i[idx].distFromCam.w;
		...
		float4 frag(g2f i) : COLOR
		{
			...
			float4 color = I * _WireColor * sin(-i.dist.w * 0.5 + _Time.x * 20) * sin(-i.dist.w * 2 + _Time.x * 20) + (1 - I) * _BaseColor;
			//float4 color = I * _WireColor * sin(_Time.x * 100 * (1/dist)) + (1 - I) * _BaseColor;
			color.a = I;
			return color;
			}
			ENDCG
